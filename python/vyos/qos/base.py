    ...
    def update(self, config, direction, priority=None):
        ...
        if 'class' in config:
            for cls, cls_config in config['class'].items():
                ...
                dscp_value = None
                if set_dscp:
                    dscp_value = self._get_dsfield(set_dscp)
                ...
                if 'match' in cls_config:
                    ...
                    for index, (match, match_config) in enumerate(cls_config['match'].items(), start=1):
                        ...
                        filter_cmd += f' protocol {filter_protocol}'
                        ...
                        if dscp_value is not None:
                            ...
                            if is_ipv4:
                                dscp_action = (
                                    f' action pedit ex munge ip dsfield set {dscp_value} retain 0xfc'
                                    f' pipe action csum ip4h'
                                )
                            elif is_ipv6:
        ...
        if 'default' in config:
            ...
            if set_dscp and self.qostype == 'shaper':
                dscp_value = self._get_dsfield(set_dscp)
                filter_cmd = (
                    f'tc filter replace dev {self._interface} parent {self._parent:x}:'
                )
                filter_cmd += f' prio 255 protocol ip u32 match u32 0 0'
                filter_cmd += (
                    f' action pedit ex munge ip dsfield set {dscp_value} retain 0xfc'
                    f' pipe action csum ip4h'
                )
                ...
                filter_cmd += f' prio 256 protocol ipv6 basic'
                filter_cmd += f' action pedit ex munge ip6 traffic_class set {dscp_value} retain 0xfc'
                filter_cmd += f' flowid {self._parent:x}:{default_cls_id:x}'
                self._cmd(filter_cmd)
